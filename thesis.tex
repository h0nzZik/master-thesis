...
Every enumeration type has associated an \texttt{underlying type}. It is an integral type, which can represent all values of the enumeration; it also defines the size of the enumeration\footnote{Technically, it is more complicated: \url{https://stackoverflow.com/q/47444081/6209703}}. A programmer may find out the underlying type of an enumeration using the template \lstinline{std::underlying_type_t<T>}{}, which is provided by the standard library.

An enumeration's underlying type can be explicitly specified \theStandard{\S 7.2:5}. This is achieved by appending a colon followed by a name of an integral type right after the \texttt{enum-name} in the declaration, as shown in Figure~\ref{fig:explicitlySpecifiedUnderlyingType}. The most obvious reason why would programmer want to specify an enum's underlying type is in order to limit the enumeration's size, perhaps because of hardware constrains. Before C++11, this was often achieved through an extension of a particular compiler; for example, the GNU gcc compiler provides a special attribute \texttt{packed}, which causes an enumeration's size to be as small as possible (Figure~\ref{fig:packedEnum}).
