...
In \K framework, languages are described using small-step operational semantics. This kind of semantics assigns to every program a transition system $( \var{Cfg}, \rightarrow )$, where $\Var{Cfg}$ is a set of program configurations and $\rightarrow$ is a binary relation over $\var{Cfg}$. The relation $\rightarrow$ is called a ,,transition relation'' and its elements are ,,transitions''. Transitions correspond to computational steps; existence of transitions between given configurations is determined e.g. by a deductive system (in case of structural operational semantics), or by pattern matching rewriting rules to the configurations (in case of \K).
